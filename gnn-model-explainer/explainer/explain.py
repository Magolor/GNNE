...
class Explainer:
    def __init__(
        ...
        self.n_hops = args.num_gc_layers if args is not None else None
        self.graph_mode = graph_mode
        self.graph_idx = graph_idx
        self.neighborhoods = None if self.graph_mode or adj is None else graph_utils.neighborhoods(adj=self.adj, n_hops=self.n_hops, use_cuda=use_cuda)
        self.args = args
        self.writer = writer
    ...
    def stats(self, args, nodes):
        if len(nodes)==0:
            return {'data':[[],[]],'AUROC':0.0}
        masked_adjs = []
        for i,node in zip(tqdm.trange(len(nodes)),nodes):
            masked_adjs.append(self.explain(node, graph_idx=-1))
        graphs = []; feats = []; adjs = []; pred_all = []; real_all = []; stat = {}
        for i, idx in enumerate(nodes):
            ...
            pred, real = self.make_pred_real(masked_adjs[i], new_idx, idx, args.syn_type)
        ...
        stat['data'] = [pred_all, real_all]
        stat['AUROC'] = roc_auc_score(real_all, pred_all)
        precision, recall, thresholds = precision_recall_curve(real_all, pred_all)
        # plt.figure(); plt.plot(recall, precision); plt.savefig("./workspace/ROCcurve.png"); plt.close()
        return stat
    ...
    def make_pred_real(self, adj, start, old, syntype):
        ...
        for item in shape_adj[shape]:
            real[start+item[0]][start+item[1]]=10
