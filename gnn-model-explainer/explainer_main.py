...
def arg_parse():
    ...
    parser.add_argument(
        ...
        const=True,
        default=False,
        help="whether to generate statistic analysis.",
    )
...
def main():
    ...
    #     print("Using CUDA", prog_args.cuda, end=': ')
    # else:
    #     print("Using CPU", end=': ')
    ...
    if prog_args.stat:
        pred = torch.load("./workspace/GNN-{0}-prediction.pkl".format(prog_args.name_suffix))
        label = torch.load("./workspace/labeled_nodes_{0}.pkl".format(prog_args.name_suffix))
        nodes = [v for v in torch.load("./workspace/nodes-{0}-f=0.tmp".format(prog_args.name_suffix)) if int(pred[v])==int(label[v])]
        torch.save(explainer.stats(prog_args,nodes),"./workspace/GNNE-{0}-f=0.stat".format(prog_args.name_suffix))
        os.remove("./workspace/nodes-{0}-f=0.tmp".format(prog_args.name_suffix))
        nodes = [v for v in torch.load("./workspace/nodes-{0}-f=1.tmp".format(prog_args.name_suffix)) if int(pred[v])==int(label[v])]
        torch.save(explainer.stats(prog_args,nodes),"./workspace/GNNE-{0}-f=1.stat".format(prog_args.name_suffix))
        os.remove("./workspace/nodes-{0}-f=1.tmp".format(prog_args.name_suffix))
    elif prog_args.explain_node is not None:
